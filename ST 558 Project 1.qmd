...
## plot.state function
This function plots the mean values of enrollment by year for each Division, after removing the Divisions with Error values. First, the function filters out the Division values not equal to "Error". Then, it groups the tibble's summary statistics by Division and Year_Date
```{r}
plot.state <- function(df, values = "Enrollment"){
  ...
  aes(x = Year_Date, y = mean_enrollment, color = Division, linetype = Division))
#create a line plot with ggplot with appropriate aes() assignments
G + geom_line() + labs(x = "Year", y = "Mean Enrollment", title = "Mean Enrollment by Division over Time")
}

# Test running the code (to be deleted later)
plot.state(edu_a$`Non-County Data`)
```
## plot.county Function
This function will take in a tibble/dataframe, along with a specific state (specified with a 2-character string), conditional logic to choose the head(, n = ) or tail(, n = ) of the dataframe 
```{r}
...
# Test running the code (to be deleted later)
plot.county(edu_a$`County Data`, input_state = "NC", choice = "bott", num_obs = 25)
```
